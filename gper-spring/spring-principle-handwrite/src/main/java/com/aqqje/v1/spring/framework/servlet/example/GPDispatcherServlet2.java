package com.aqqje.v1.spring.framework.servlet.example;
...
//        if(!this.handlerMapping.containsKey(url)){
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        System.out.println("AQ Spring framework is init.");
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            if(!clazz.isAnnotationPresent(AQController.class)){continue;}


            //保存写在类上面的@AQRequestMapping("/demo")
            String baseUrl = "";
            if(clazz.isAnnotationPresent(AQRequestMapping.class)){
                AQRequestMapping requestMapping = clazz.getAnnotation(AQRequestMapping.class);
                baseUrl = requestMapping.value();
            }

            //默认获取所有的public方法
            for (Method method : clazz.getMethods()) {
                if(!method.isAnnotationPresent(AQRequestMapping.class)){continue;}

                AQRequestMapping requestMapping = method.getAnnotation(AQRequestMapping.class);
                //优化
                // //demo///query
    ...
    private void doAutowired() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (Field field : fields) {
                if(!field.isAnnotationPresent(AQAutowired.class)){continue;}
                AQAutowired autowired = field.getAnnotation(AQAutowired.class);
        ...
        try {
            for (String className : classNames) {
                ...
                if(clazz.isAnnotationPresent(AQController.class)){
                    ...
                    //1、自定义的beanName
                    AQService service = clazz.getAnnotation(AQService.class);
        ...
        private void putParamIndexMapping(Method method){
            ...
            for (int i = 0; i < pa.length ; i ++) {
                for(Annotation a : pa[i]){
                    if(a instanceof AQRequestParam){
                        String paramName = ((AQRequestParam) a).value();
