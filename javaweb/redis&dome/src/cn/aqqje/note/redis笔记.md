...
##### 2.1 [官网](https://redis.io)

##### 2.2 [中文网](http://www.redis.net.cn/)
...
```text
1 添加：
    A. lpush key value: 将元素加入列表左表
        
    B. rpush key value：将元素加入列表右边
...
在一定的间隔时间中，检测key的变化情况，然后持久化数据
    ...
    2 重新启动redis服务器，并指定配置文件名称
        C:\LearnEnv\Redis>redis-server.exe redis.windows.conf	
```
...
```text
    Jedis.lpush() / Jedis.rpush() :存储
    Jedis.lpop() / Jedis.rpop(): 弹出
    Jedis.lrange(start end) : 范围获取
